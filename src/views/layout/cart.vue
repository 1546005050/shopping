  ...
  <div class="cart">
    ...
    <div class="cart-list">
      <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
        <van-checkbox @click="toggleCk(item.goods_id)" :value="item.isCheacked"></van-checkbox>
        <div class="show">
    ...
    <div class="footer-fixed">
      ...
      <div class="all-total">
        ...
        <div v-else @click="delcart" class="delete" :class="{disabled:selCount===0}">删除({{ selCount }})</div>
      </div>
    </div>
    </div>
...
export default {
  ...
  methods: {
    toggleCk (goodsId) {
      this.$store.commit('cart/toggleCk', goodsId)
    },
    toggleAllCheck () {
    ...
    goPay () {
      ...
      if (this.selCount > 0) {
        ...
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selectList.map(item => item.id).join(',')// 'cartId,cartId,cartId'
          }
        })
      }
    }
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selectList', 'selCount', 'selPrice', 'isAllchecked']),
    isLogin () {
